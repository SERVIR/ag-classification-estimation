...
var num_years = [2016,2017,2018,2019,2020];
var year = "2016"; // "2020"
...
function gen_cropped_rice_map(){
  IC = ee.Image(imgList.get(yr_idx)).clip(ROI)
}
...
function set_stats(){
  ...
  filt = district_stats.filter(ee.Filter.eq('Year', s_year)).filter(ee.Filter.eq('Dzongkhag', dist)).first().get('Harvested area(acres)').getInfo();
  if (dist == "All Dzongkhags"){
    
    message = ui.Panel([
    ui.Label("Total harvested rice area in Bhutan for "+year+ " according to DOA statistics: "+  filt +" acres",{fontWeight: 'bold'}),
...
function updateMapPanel(not_date, not_sub, subdist) {
  ...
  if (year == "2019" || year == "2020"){
    ...
    loss_chart = ui.Label('Insuffient rice maps after the selected year to compare loss to',{backgroundColor:'red'});
    panel.insert(12,loss_chart)
    panel.remove(gain_chart)
    gain_chart = ui.Label('Insuffient rice maps after the selected year to compare gain to',{backgroundColor:'red'});
...
var subdistrict_select = ui.Select({
  ...
  onChange: function (subdistrict_selection) {
    if (subdistrict_selection == "All Gewogs"){
      ...
      ROI = BT2.filterMetadata('ADM2_EN','equals',subdistrict_selection)
    }
...
var district_select = ui.Select({
  ...
  onChange: function (district_selection) {
    if (district_selection == "All Dzongkhags"){
      ...
      ROI = BT2.filterMetadata('ADM1_EN','equals',district_selection)
      ...
      var selected_subdistrict = SubDistrictNames.evaluate(function(values) {
...
label = ui.Label('The graph below shows the rice area in acres in the area of interest from 2016 to 2020.');
panel.add(label)
gen_rice_chart();
panel.add(IC_chart);

label = ui.Label('The graph below shows the average rice area in acres produced from aggregated annual crop masks from 2016 to 2020. By default, the regions are distributed by Dzongkhags. If a particular Dzongkhag is selected as the region of interest, the graph shows the distribution by Gewogs.');
panel.add(label)
